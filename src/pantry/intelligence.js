...
export function calculateAvailableServings(item={}){
 if(!yes(item.on_hand??'Yes')||['depleted','discarded','expired'].includes(String(item.status||'').toLowerCase()))return 0;
...
export function pantryHealthScore(items=[],events=[],now=Date.now()){
 const active=items.filter(i=>calculateAvailableServings(i)>0);if(!active.length)return {score:0,label:'Empty',components:{freshness:0,variety:0,protein:0,fiber:0,waste:0,confidence:0}};
...
export function buildPantryIntelligence({items=[],events=[],plannedMeals=[],history=[],purchases=[],currentLocation='Home',now=Date.now()}){
 const enriched=items.map(item=>{const forecast=forecastPantry(item,plannedMeals,history,new Date(now)),freshness=freshnessStatus(item,now),confidence=pantryConfidence(item,events,now),waste=wasteRisk(item,events,now),cost=costMetrics(item),locationMatch=(()=>{const selected=String(currentLocation||'Home').toLowerCase(),location=String(item.location||'').toLowerCase();return selected==='home'?['home','refrigerator','freezer','pantry','garage refrigerator','wine cellar','standalone freezer','standalone refrigerator'].some(x=>location===x||location.includes(x)):location.includes(selected)})();const opportunity=opportunityScore({...item,current_location_match:locationMatch?'yes':'no'},events,now);return {...item,available:calculateAvailableServings(item)>0,remainingServings:calculateAvailableServings(item),confidence,freshness,waste,forecast,cost,locationMatch,opportunity,verificationState:confidence>=75?'Verified on hand':confidence>=45?'Probably on hand':calculateAvailableServings(item)<=0?'Out of stock':'Verify'}}).sort((a,b)=>b.opportunity.score-a.opportunity.score||b.waste.score-a.waste.score);
 const shopping=optimizeGroceries({items:enriched,plannedMeals,history});const restock=enriched.filter(i=>i.forecast.daysRemaining!=null&&i.forecast.daysRemaining<=7).sort((a,b)=>a.forecast.daysRemaining-b.forecast.daysRemaining);
 const health=pantryHealthScore(enriched,events,now);enriched.items=enriched;enriched.recommendations=enriched.filter(i=>i.available&&i.locationMatch).slice(0,8);enriched.wasteRisks=enriched.filter(i=>i.waste.score>=30).sort((a,b)=>b.waste.score-a.waste.score);enriched.restock=restock;enriched.shopping=shopping;enriched.health=health;enriched.currentLocation=currentLocation;return enriched;
}
