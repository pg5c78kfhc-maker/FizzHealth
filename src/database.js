...
,  {version:99,name:'complete_2026_laboratory_panel_and_align_labs_ui',sql:`
  ...
  {version:128,name:'Podcast Information Context Menus and Playlist Reordering',sql:`
    CREATE TABLE IF NOT EXISTS podcast_playlist_podcast_order (
      playlist_id TEXT NOT NULL,
      podcast_id TEXT NOT NULL,
...
function ensureAudibleSeedData(){
  ...
  if(hasTable('release_metadata'))db.run(`INSERT OR REPLACE INTO release_metadata(version,release_date,build_id,schema_version,title,created_at) VALUES ('1.4.17.21','2026-08-08','141721',147,'Audible Refresh, Runtime Formatting & Library Expansion','2026-08-08T08:45:00-04:00')`);
}
...
export async function transaction(callback,{operation='database-write',transactionId=''}={}){
 const execute=async()=>{const id=transactionId||`tx-${Date.now()}-${++transactionSequence}`,backup=db.export(),startedAt=Date.now();let committed=false,rolledBack=false,tx=null;try{db.run('BEGIN IMMEDIATE TRANSACTION');tx={transactionId:id,operation,diagnostic:null,run:(sql,params=[])=>db.run(sql,normalizeParams(params)),query};const result=await callback(tx);if(tx.diagnostic)Object.assign(tx.diagnostic,{commitRequestedAt:new Date().toISOString()});db.run('COMMIT');committed=true;await persist();if(tx.diagnostic)Object.assign(tx.diagnostic,{commitResult:'COMMITTED',rollbackResult:'NOT_REQUIRED',commitCompletedAt:new Date().toISOString(),durationMs:Date.now()-startedAt});return result}catch(error){try{db.run('ROLLBACK');rolledBack=true}catch{}if(!committed){db=new SQL.Database(new Uint8Array(backup));await persist()}if(tx?.diagnostic)Object.assign(tx.diagnostic,{commitResult:'FAILED',rollbackResult:rolledBack?'ROLLED_BACK':'DATABASE_RESTORED',durationMs:Date.now()-startedAt,exception:error?.message||String(error),stackTrace:String(error?.stack||'')});error.transactionId=error.transactionId||id;error.transactionOperation=error.transactionOperation||operation;error.commitResult=committed?'COMMITTED':'FAILED';error.rollbackResult=rolledBack?'ROLLED_BACK':'DATABASE_RESTORED';throw error}};
