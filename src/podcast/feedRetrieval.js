...
export async function retrievePodcastFeed({feedUrl,appleId='',fallbackMetadata={},fetchImpl=globalThis.fetch,storage=globalThis.localStorage,proxyUrl='/api/podcast-feed',timeoutMs=15000,onStatus=()=>{},onDiagnostic=()=>{},onRecoveredFeedUrl=async()=>{},recoveryAttempted=false}){
 ...
 try{
  ...
  const proxyPayload=error?.proxyPayload||null,upstreamStatus=Number(proxyPayload?.upstreamStatus||proxyPayload?.upstream_status||proxyPayload?.statusCode||0),redirectKind=String(proxyPayload?.redirectType||proxyPayload?.redirect_type||'').toLowerCase(),advertised=safePublicUrl(proxyPayload?.appleFeedUrl||proxyPayload?.apple_feed_url||proxyPayload?.directoryFeedUrl||'');
  record({stage:'proxy',success:false,url:`${proxyUrl}?url=${encodeURIComponent(url)}`,feedUrl:url,durationMs:Date.now()-started,...errorData(error),contentType:error?.contentType||'',finalUrl:error?.finalUrl||'',proxyPayload,upstreamStatus});
  const recoverable=(upstreamStatus===404||upstreamStatus===410||redirectKind==='permanent'||Boolean(proxyPayload?.permanentRedirect));
  if(!recoveryAttempted&&appleId&&recoverable&&advertised&&advertised!==url){record({stage:'apple-recovery',success:true,appleId,previousFeedUrl:url,advertisedFeedUrl:advertised,result:'RECOVERED'});await onRecoveredFeedUrl({feedUrl:advertised,previousFeedUrl:url,appleId,upstreamStatus});return retrievePodcastFeed({feedUrl:advertised,appleId,fallbackMetadata,fetchImpl,storage,proxyUrl,timeoutMs,onStatus,onDiagnostic,onRecoveredFeedUrl,recoveryAttempted:true})}
  if(recoverable&&advertised===url)record({stage:'apple-recovery',success:false,appleId,advertisedFeedUrl:advertised,result:'APPLE_ADVERTISES_DEAD_URL'});
  else if(recoverable&&!advertised)record({stage:'apple-recovery',success:false,appleId,result:'LOOKUP_FAILED'});
  throw Object.assign(new Error('Couldn’t retrieve this podcast feed.'),{code:'feed-retrieval-failed',diagnostics,cause:error,requestedUrl:url,urlIntegrity:recoverable?(advertised===url?'APPLE_ADVERTISES_DEAD_URL':'LOOKUP_FAILED'):'NOT_CHECKED'})
 }
}
