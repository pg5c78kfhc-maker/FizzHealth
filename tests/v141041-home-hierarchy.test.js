...
test('existing intelligence is revealed inside the expandable region',()=>{
 const disclosure=main.slice(main.indexOf('decision-intelligence-disclosure'),main.indexOf('home-key-summaries'));
...
test('release metadata is v1.4.10.41',()=>{
 assert.equal(meta.version,'1.4.10.41');
 assert.equal(meta.build,'141041');
 assert.match(main,/const VERSION='1\.4\.10\.41'/);
 assert.match(main,/const BUILD_ID='141041'/);
});
