...
test('planner displays restaurant-style saved menu and starred restaurant catalog',()=>{assert.match(main,/TODAY’S MENU/);assert.match(main,/Today’s Specials/);assert.match(main,/Chef’s Picks/);assert.match(main,/STARRED RESTAURANT MEALS/);assert.match(main,/COALESCE\(rm.favorite,0\)=1/)});
test('release metadata is current',()=>{assert.equal(meta.version,'1.4.11.22');assert.equal(meta.schema_version,54);assert.match(db,/version:52/)});
