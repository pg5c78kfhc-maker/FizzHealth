...
test('planner uses saved meals and date calendar',()=>{assert.match(main,/SELECT \* FROM meal_definitions/);assert.match(main,/MEAL PLANNING PROTOTYPE/);assert.match(main,/Reserve capacity/);assert.doesNotMatch(main,/Plan from pantry foods, recipes, leftovers/)});
...
test('release metadata is current',()=>{assert.equal(meta.version,'1.4.11.14');assert.equal(meta.schema_version,51)});
