...
test('release metadata advances to v1.4.11.17',()=>{assert.equal(meta.version,'1.4.11.17');assert.equal(meta.build,'141317');assert.match(main,/const VERSION='1\.4\.11\.17'/)});
...
test('restaurant cards show price and labeled confidence',()=>{assert.match(main,/restaurant-price-badge/);assert.match(main,/Price unknown/);assert.match(main,/>Confidence<\/small>/);assert.match(css,/restaurant-price-badge/)});
