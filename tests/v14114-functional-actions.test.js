...
test('v1.4.11.8 metadata is current everywhere',()=>{
  assert.equal(meta.version,'1.4.11.21');
  assert.equal(meta.build,'141121');
  assert.equal(meta.release_id,'FH-20260724-141121');
  assert.match(main,/const VERSION='1\.4\.11\.21'/);
  assert.match(main,/const BUILD_ID='141121'/);
  assert.match(main,/const DEPLOYMENT_ID='FH-20260724-141121'/);
});

test('pantry detail pencil opens the real editor without dismissing the detail first',()=>{
  assert.match(main,/onClick=\{e=>\{e\.stopPropagation\(\);setSelectedPantry\(scoreItem\)\}\}/);
