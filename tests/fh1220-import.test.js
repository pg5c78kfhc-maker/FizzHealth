...
test('release metadata preserves schema 43 in v1.4.10.15',()=>{
  const meta=JSON.parse(fs.readFileSync('VERSION.json','utf8'));
  assert.equal(meta.version,'1.4.10.15');assert.equal(meta.schema_version,43);assert.ok(meta.stories.includes('FH-1227'));
});
