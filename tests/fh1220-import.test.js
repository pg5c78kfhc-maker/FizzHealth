...
test('release metadata preserves schema 41 in v1.4.10.13',()=>{
  const meta=JSON.parse(fs.readFileSync('VERSION.json','utf8'));
  assert.equal(meta.version,'1.4.10.13');assert.equal(meta.schema_version,41);assert.ok(meta.stories.includes('FH-1227'));
});
