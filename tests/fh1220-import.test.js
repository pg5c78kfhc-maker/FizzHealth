...
test('FH-1220 schema and importer coverage are present',()=>{
  ...
  assert.match(db,/TARGET_SCHEMA_VERSION=42/);
...
test('release metadata preserves schema 42 in v1.4.10.14',()=>{
  const meta=JSON.parse(fs.readFileSync('VERSION.json','utf8'));
  assert.equal(meta.version,'1.4.10.14');assert.equal(meta.schema_version,42);assert.ok(meta.stories.includes('FH-1227'));
});
