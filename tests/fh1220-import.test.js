...
test('release metadata preserves schema 40 in v1.4.10.10',()=>{
  const meta=JSON.parse(fs.readFileSync('VERSION.json','utf8'));
  assert.equal(meta.version,'1.4.10.10');assert.equal(meta.schema_version,40);assert.ok(meta.stories.includes('FH-1227'));
});
