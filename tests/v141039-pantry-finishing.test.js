...
test('v1.4.10.39 identity is canonical',()=>{
 assert.match(main,/const VERSION='1\.4\.10\.39'/);
 assert.match(main,/const BUILD_ID='141039'/);
});
test('missing-data cards use icon-only fixed status column and distinct background',()=>{
 ...
 assert.match(css,/\.pantry-smart-item\.needs-data-card\{background:/);
});
